...
const mockData = {
	questions: [
		{ id: 1, text: 'First question', views: 0 },
...
describe('+page.svelte', () => {
	it('renders page title and question data', async () => {
		render(Page, { data: mockData });

		const heading = page.getByRole('heading', { level: 1, name: 'Questions' });
		await expect.element(heading).toBeInTheDocument();

		const questionItem = page.getByText('First question');
		await expect.element(questionItem).toBeInTheDocument;

		const secondQuestionItem = page.getByText('Second question');
